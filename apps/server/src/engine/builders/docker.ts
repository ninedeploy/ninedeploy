...
function isGeneratedNixpacksToml(tomlPath: string): boolean {
  try {
    return readFileSync(tomlPath, 'utf8').startsWith(GENERATED_NIXPACKS_MARKER);
  } catch {
    return false;
  }
}
...
async function buildWithNixpacks(
  ...
  if (toml) {
    ...
    if (existsSync(tomlPath) && !isGeneratedNixpacksToml(tomlPath)) {
    ...
    try {
      const tomlPath = resolveInRepo(workDir, baseDir, 'nixpacks.toml');
      if (isGeneratedNixpacksToml(tomlPath)) {
