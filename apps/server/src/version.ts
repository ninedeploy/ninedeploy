...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    changes: [
      ...
      'Reusing a deleted service’s slug no longer mounts its retained data volume, git checkouts pin the addresses the egress gate vetted (DNS rebinding), SAML — which could never sign anyone in — is refused honestly, backup drills verify dumps inside the database’s own image instead of with host tools no installer provides, and Traefik is recreated when it serves another data directory’s config.',
      'The installer survives tty-less self-updates, rolls a failed upgrade back to the previous release (database included), migrates the real database instead of a stray one, and its --docker mode works with pinned versions. The panel gained domain verification, deployment-lane selection, paginated activity, a working Web Studio link, a domain-transfer accept page, an enrolment-token card, visible branding and keyboard-accessible dialogs, and a dozen stale-cache and contract mismatches between the web, SDK, CLI and MCP were fixed.',
    ],
  },
  {
    version: '0.10.7',
