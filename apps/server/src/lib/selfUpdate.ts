...
function updaterScript(p: SelfUpdatePaths, installDir: string): string {
  ...
  return [
    ...
    // its fix. Fetched from the tag itself (the same bytes the tarball will
    // carry; `curl …/install.sh | bash` is the documented upgrade path), and
    // checked for a bash shebang and a clean `bash -n` parse. Any failure
    // falls back to the installed installer, so the fetch can only help.
    'installer=./install.sh',
    `rm -f "${fetched}" "${fetched}.part"`,
    [
      `if curl -fsSL --retry 3 --retry-delay 2 -m 120 -o "${fetched}.part" "https://raw.githubusercontent.com/${INSTALLER_REPO}/$ND_SELF_UPDATE_TARGET/install.sh"`,
      `head -n 1 "${fetched}.part" | grep -q '^#!.*bash'`,
      `bash -n "${fetched}.part"`,
      `mv -f "${fetched}.part" "${fetched}"; then`,
    ].join(' && '),
    `  installer="${fetched}"`,
    '  echo "== using the $ND_SELF_UPDATE_TARGET installer"',
    'else',
    `  rm -f "${fetched}.part"`,
    '  echo "== could not fetch the $ND_SELF_UPDATE_TARGET installer — using the installed one"',
    'fi',
